 ...
 */
public class XMLFolderTranslatorIntegrationTest {
	DRNLPFactory factory;
	FolderParser parser;
	Path path;
	XMLFolderTranslator xmlFolderTranslator;
	...
	public void setUp() throws Exception {
		...
		xmlFolderTranslator = new XMLFolderTranslator();
	...
	public void testTranslateToString() throws Exception {
		...
		String xml = xmlFolderTranslator.translateToString(folder);
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		DocumentBuilder builder = factory.newDocumentBuilder();
		InputSource is = new InputSource(new StringReader(xml));
		org.w3c.dom.Document xmlDoc = builder.parse(is);
		assertThat(xmlDoc.getElementsByTagName("folder").getLength(),
				greaterThan(0));
		assertThat(xmlDoc.getElementsByTagName("documents").getLength(),
				greaterThan(0));
		...
		// assert documents are children of folder
		nodeList = xmlDoc.getElementsByTagName("folder");
		node = nodeList.item(0);
		childrenNodes = node.getChildNodes();
		found = false;
		for (int i = 0; i < childrenNodes.getLength(); i++)
		{
			if (childrenNodes.item(i).getNodeName().equals("documents"))
			{
				found = true;
			}
		}
		// assert document are children of documents
		nodeList = xmlDoc.getElementsByTagName("documents");
		node = nodeList.item(0);
		childrenNodes = node.getChildNodes();
		found = false;
		for (int i = 0; i < childrenNodes.getLength(); i++)
		{
			if (childrenNodes.item(i).getNodeName().equals("document"))
			{
				found = true;
			}
		}
